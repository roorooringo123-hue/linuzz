...
const loadTasks = (): Task[] => {
  try {
    const saved = localStorage.getItem('jcimlasos_todos');
    if (saved) return JSON.parse(saved);
  } catch { /* ignore */ }
  const today = new Date().toISOString().split('T')[0];
  return [
    { id: generateId(), title: 'Explore the jcimlasOS desktop', completed: false, dueDate: today, priority: 'high', tags: ['welcome'], projectId: 'inbox', createdAt: Date.now() },
...
const loadCustomProjects = (): Project[] => {
  try {
    const saved = localStorage.getItem('jcimlasos_todo_projects');
    if (saved) return JSON.parse(saved);
  } catch { /* ignore */ }
  return [];
};
...
const Todo: React.FC = () => {
  ...
  useEffect(() => {
    localStorage.setItem('jcimlasos_todos', JSON.stringify(tasks));
  }, [tasks]);

  useEffect(() => {
    localStorage.setItem('jcimlasos_todo_projects', JSON.stringify(customProjects));
  }, [customProjects]);
