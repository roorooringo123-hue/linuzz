...
const loadNotes = (): Note[] => {
  try {
    const saved = localStorage.getItem('jcimlasos_notes');
...
const loadFolders = (): FolderItem[] => {
  try {
    const saved = localStorage.getItem('jcimlasos_note_folders');
    if (saved) return JSON.parse(saved);
  } catch { /* ignore */ }
...
const Notes: React.FC = () => {
  ...
  useEffect(() => {
    localStorage.setItem('jcimlasos_notes', JSON.stringify(notes));
  }, [notes]);

  useEffect(() => {
    localStorage.setItem('jcimlasos_note_folders', JSON.stringify(folders));
  }, [folders]);
