...
const Settings: React.FC = () => {
  ...
  const [settings, setSettings] = useState<Record<string, unknown>>(() => {
    try { return JSON.parse(localStorage.getItem('jcimlasos_settings') || '{}'); } catch { return {}; }
  });

  const updateSetting = useCallback((key: string, value: unknown) => {
    setSettings(prev => {
      const next = { ...prev, [key]: value };
      localStorage.setItem('jcimlasos_settings', JSON.stringify(next));
      return next;
    });
  }, []);
        ...
        return (
          <div className="space-y-6">
            ...
            <div className="flex flex-col items-center py-6">
              ...
              <div className="text-xl font-semibold text-[var(--text-primary)]">jcimlasOS Web</div>
              <div className="text-sm text-[var(--text-secondary)]">Version 24.04 LTS</div>
            </div>
            <div className="space-y-3">
              {[
                ['Device Name', 'jcimlasos-desktop'],
                ['Memory', '8 GB'],
