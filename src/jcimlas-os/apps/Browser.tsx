...
const NEWS_ARTICLES = [
  { title: 'jcimlas 24.04 LTS Released with New Features', source: 'jcimlas.com', time: '2h ago' },
...
// ---- Simulated pages ----
const IFRAME_FRIENDLY_SITES = ['example.com', 'wikipedia.org', 'jcimlas.com'];
...
const generateSimulatedPage = (url: string): string => {
  ...
  return `
    ...
    <body>
      <div class="header">
        <h1>${host}</h1>
        <p>Simulated page for jcimlasOS Browser</p>
      </div>
      <div class="content">
        <h2>Welcome to ${host}</h2>
        <p>This is a simulated version of the website running inside jcimlasOS Browser. Many real websites block iframe embedding for security reasons.</p>
  ...
  return (
    <div className="h-full flex flex-col items-center justify-start pt-12 custom-scrollbar overflow-auto" style={{ background: 'var(--bg-window)' }}>
      <div className="flex items-center gap-3 mb-8">
        ...
        <h1 style={{ fontSize: '28px', fontWeight: 700, color: 'var(--text-primary)' }}>jcimlasOS Browser</h1>
      </div>
...
export default function Browser() {
  ...
  const [bookmarks, setBookmarks] = useState<Bookmark[]>(() => {
    try {
      const saved = localStorage.getItem('jcimlasos_browser_bookmarks');
  ...
  useEffect(() => {
    localStorage.setItem('jcimlasos_browser_bookmarks', JSON.stringify(bookmarks));
  }, [bookmarks]);
